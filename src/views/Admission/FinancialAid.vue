        ...
        <div class="absolute inset-0 z-10 flex flex-col justify-end pointer-events-none">
            <div class="max-w-7xl mx-auto w-full px-6 sm:px-8 lg:px-16 pb-12 md:pb-16 reveal-group">
                ...
                <p class="text-base md:text-lg text-white/60 max-w-xl leading-relaxed reveal-item">
                    Explore available scholarships and financial assistance programs for SFXC
                    students.
                </p>
            </div>
        </div>
    ...
    <div class="bg-background">
        <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-16 py-24 space-y-16">
            <div class="reveal-group">
                ...
                <p class="text-lg text-muted-foreground leading-relaxed mb-6 max-w-3xl reveal-item">
                    Education at SFXC is an investment in our students’ futures. As our institution
                    continues to strive for the socio-economic development of society, we believe
                    our students deserve quality education. With many of our students coming from
                    humble backgrounds, the Admission and Scholarship Office (ADSO) is more than
                    ready to assist with financial aid for our deserving students. <br /><br />
                    Below is a list of available and accepted financial aid at SFXC. Should there be
                    other scholarships and grants that are not mentioned, contact ADSO for special
                    arrangements.
                </p>
                <div class="mt-6 space-y-2 text-sm md:text-base text-muted-foreground reveal-item">
                    <p class="text-base md:text-lg font-bold text-foreground">
                        𝐀𝐃𝐌𝐈𝐒𝐒𝐈𝐎𝐍 𝐀𝐍𝐃 𝐒𝐂𝐇𝐎𝐋𝐀𝐑𝐒𝐇𝐈𝐏 𝐎𝐅𝐅𝐈𝐂𝐄
                    </p>
                    <p class="wrap-break-word">
                        <span class="font-semibold text-foreground">Facebook:</span> SFXC
                        Scholarship Office
                    </p>
                    <p class="wrap-break-word">
                        <span class="font-semibold text-foreground">Email:</span>
                        Admission.scholarship@sfxc.edu.ph
                    </p>
                    <p class="wrap-break-word">
                        <span class="font-semibold text-foreground">Mobile:</span> 0908 885 5380
                    </p>
                </div>
            </div>
            ...
            <div class="space-y-6 reveal-group">
                ...
                <div class="space-y-3 reveal-item">
                    ...
                    >
                        ...
                        >
                            <Minus
                                v-if="expanded[aid.name]"
                                class="w-4 h-4 shrink-0 text-foreground"
                                stroke-width="2.5"
                            />
                            <Plus
                                v-else
                                class="w-4 h-4 shrink-0 text-foreground text-opacity-80"
                                stroke-width="2.5"
                            />
                            {{ aid.name }}
                        </button>
                        ...
                        <div
                            v-show="expanded[aid.name]"
                            class="p-6 border-t border-border bg-background"
                        >
                            <div class="mb-6">
                                <div
                                    v-if="aid.amount"
                                ...
                                <p
                                    class="text-muted-foreground leading-relaxed text-sm md:text-base whitespace-pre-wrap"
                                >
                                    {{ aid.description }}
                                </p>
                            </div>
                            ...
                            <div class="gap-8 grid md:grid-cols-2">
                                <div v-if="aid.requirements && aid.requirements.length > 0">
                                    <h4
                                        class="text-sm font-bold text-foreground mb-3 tracking-wider flex items-center gap-2"
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="16"
                                    ...
                                    <ul class="space-y-3 text-sm">
                                        <li
                                            v-for="(req, idx) in parseRequirements(
                                                aid.requirements,
                                            )"
                                            :key="idx"
                                        ...
                                        >
                                            <div
                                                v-if="req.subItems"
                                                class="bg-muted/30 rounded-md border border-border"
                                            >
                                                <button
                                                ...
                                                >
                                                    ...
                                                    <ChevronRight
                                                        :class="[
                                                            ...
                                                            nestedExpanded[`${aid.name}-req-${idx}`]
                                                                ? 'rotate-90'
                                                                : '',
                                                        ]"
                                                    />
                                                ...
                                                >
                                                    <ul
                                                        class="space-y-2 ml-2 border-l-2 border-primary/20 pl-4 py-1"
                                                    >
                                                        <li
                                            ...
                                            <div v-else class="flex items-start gap-2 pt-1">
                                                <span
                                                    class="shrink-0 w-1.5 h-1.5 rounded-full bg-primary mt-1.5"
                                                ></span>
                                ...
                                <div v-if="aid.applicationSteps && aid.applicationSteps.length > 0">
                                    ...
                                    <ul
                                        class="list-decimal list-outside ml-4 space-y-2 text-muted-foreground text-sm"
                                    >
                                        <li
                                            v-for="(step, idx) in aid.applicationSteps"
                                            :key="idx"
            ...
            <div class="space-y-6 reveal-group">
                ...
                <p class="text-muted-foreground leading-relaxed max-w-3xl reveal-item">
                    The application for the following Government-Funded Scholarship are
                    agency-direct. Scholars need to submit their applications directly to the
                    scholarship giving agency and transact scholarship concerns like requirements
                    and allowances straight from their office. However, announcements and updates
                    regarding various scholarships offers shall be posted by the ADSO once
                    requested.
                </p>

                <div class="space-y-3 reveal-item">
                    <div
                        v-for="(aid, index) in otherGovPrograms"
                        :key="aid.name"
                        class="border border-border rounded-md overflow-hidden bg-background"
                    >
                        <button
                            ...
                            class="w-full flex items-center gap-3 p-4 bg-muted/20 hover:bg-muted/50 transition-colors text-left font-semibold text-foreground"
                        >
                            <span class="text-muted-foreground font-medium text-sm w-4"
                                >{{ index + 1 }}.</span
                            >
                            <span class="flex-1">{{ aid.name }}</span>
                            <Minus
                                v-if="expanded[aid.name]"
                                class="w-4 h-4 shrink-0 text-foreground ml-auto"
                                stroke-width="2.5"
                            />
                            <Plus
                                v-else
                                class="w-4 h-4 shrink-0 text-foreground text-opacity-80 ml-auto"
                                stroke-width="2.5"
                            />
                        </button>
